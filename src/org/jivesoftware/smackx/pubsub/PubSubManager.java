...
final public class PubSubManager
{
		...
		throws XMPPException
	{
		...
		if (node == null)
		{
			...
			DiscoverInfo infoReply = (DiscoverInfo)SyncPacketSend.getReply(con, info, WAIT);
		...
		throws XMPPException
	{
		...
		items.setTo(to);
		DiscoverItems nodeItems = (DiscoverItems)SyncPacketSend.getReply(con, items, WAIT);
		return nodeItems;
	}
	...
	static Packet sendPubsubPacket(Connection con, String to, Type type, PacketExtension ext, PubSubNamespace ns)
		throws XMPPException
	{
		return SyncPacketSend.getReply(con, createPubsubPacket(to, type, ext, ns), WAIT);
	}
	...
	static Packet sendPubsubPacket(Connection con, String to, Type type, PubSub packet, PubSubNamespace ns)
		throws XMPPException
	{
		return SyncPacketSend.getReply(con, packet, WAIT);
	}
